...
def load_and_preprocess_data():
    ...
    return X_combined, encoded_labels, label_mapping, imu_columns, gps_columns, velocity_columns


def train_and_evaluate_model(model, X_train, X_test, y_train, y_test, label_mapping, model_name):
    model.fit(X_train, y_train)
    y_pred = model.predict(X_test)
    accuracy = accuracy_score(y_test, y_pred)
    print(f'Accuracy Score ({model_name}): {accuracy * 100:.2f}%')
...
X_combined, encoded_labels, label_mapping, imu_columns, gps_columns, velocity_columns = load_and_preprocess_data()
...
for model_name, model in models.items():
    ...
    train_and_evaluate_model(model, X_train, X_test, y_train, y_test, label_mapping, model_name)
...
for model_name, model in models.items():
    ...
    plot_learning_curve(model, X_combined, np.argmax(encoded_labels, axis=1), model_name)
